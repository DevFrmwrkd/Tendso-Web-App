...
/**
 * Lead Prospects — Outscraper-scraped Google Maps businesses.
 *
 * Sibling to /admin/leads but reads from api.outscraper.listScrapedLeads
 * (which filters source === "outscraper"). Lets admins triage prospect
 * businesses pulled in via the mobile "Pull nearby businesses" action.
 *
 * Per docs/changes/BUSINESS-SCRAPER.md Surface A.
 */
...
export default function LeadProspectsPage() {
    const { isAdmin, loading: authLoading } = useAdminAuth();
    const [statusFilter, setStatusFilter] = useState<StatusFilter>("all");
    const [search, setSearch] = useState("");

    const prospects = useQuery(
        (api as any).outscraper?.listScrapedLeads,
        isAdmin ? { statusFilter, search } : "skip",
    ) as ScrapedLead[] | undefined;

    // updateStatus is a generic existing mutation — reuse so the same
    // status pipeline applies to scraped + customer leads.
    const updateStatus = useMutation((api as any).leads.updateStatus);
            ...
            <div className="space-y-6">
                ...
                <div className="flex items-start justify-between gap-6 flex-wrap">
                    <div>
                        <div className="flex items-center gap-3 mb-3">
                            <span className="ed-eyebrow">
                                Lead Prospects · {String(prospects?.length ?? 0).padStart(2, "0")} businesses
                            </span>
                        </div>
                        ...
                        <p className="ed-body mt-3 flex items-start gap-2" style={{ maxWidth: "60ch" }}>
                            <Building2 className="w-4 h-4 mt-1 flex-shrink-0" style={{ color: "var(--ed-accent)" }} />
                            <span>
                                Google-Maps-scraped prospects pulled in via the mobile app.
                                Triage them here — mark contacted as the team works the list.
                            </span>
                        </p>
                    </div>
                    <div className="text-right">
                        ...
                        <div className="ed-label mt-1">in the funnel</div>
                    </div>
                </div>
                    ...
                    <div className="ed-card-xl text-center">
                        ...
                        <p className="ed-body-sm mt-2" style={{ color: "var(--ed-ink-2)" }}>
                            {search || statusFilter !== "all"
                                ...
                                : "Run “Pull nearby businesses” from the mobile app's Leads tab to populate this list."}
                        </p>
                    </div>
                ) : (
                            ...
                            >
                                ...
                                >
                                    ...
                                    <select
                                        value={p.status}
                                        onChange={(e) => {
                                            updateStatus({ id: p._id, status: e.target.value }).catch((err: any) => {
                                                console.error(err);
                                            });
                                        }}
